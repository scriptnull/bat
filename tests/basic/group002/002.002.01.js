...
var logger = bunyan.createLogger({
  name: 'Test 2.2.1 Subscription Owner is Owner Github Public Scope'
});
...
describe('GET /subscriptions API', function () {
  ...
  it('Test 002.002.01: Checks if user is owner of their '+
      'public github subscriptions',
    function (done) {
      var account = nconf.get('testAccounts:shipaye2');
      shippable = new Shippable(nconf.get('apiEndpoint'), account.apiToken);
      githubAdapter = new GithubAdapter(account.providerToken);
      logger.info('');
      var store = {};
