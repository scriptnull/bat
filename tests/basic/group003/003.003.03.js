...
var mocha = require('mocha'),
    ...
    logger = bunyan.createLogger({ name: 'Test Case 3.3.3: ' + 
      'Using BB Token verify only public projects for the account'});

describe('Test Case 3.3.3 : Using bitbucket token,' ,
 function() {
  this.timeout(0);
  var shippable = null;
  ...
  it('verify only public projects', function(done) {
    ...
    store.privateReposReturnedByBitbucket = 0;
    ...
    async.series([
      ...
      assert.equal(store.privateReposReturnedByBitbucket,
    ...
    function getProjectsListFromShippable (store, next) {
      ...
      shippable.getAllProjects(function(err, projects) {
        ...
        logger.info('Total number of repos on shippable', projects.length);
        next();
      });
    }
      ...
      store.projects.forEach(function (project) {
        project.loadDetails(function (err, projectObject) {
          ...
          if (count === store.projects.length) {
            logger.info('Total number of private repos on shippable ',
              store.privateProjectsOnShippable);
            next();
          }
        });
      ...
      bitbucketAdapter.getDashboardRepositories(
        ...
        subscriptionAndRepos.forEach(function (subscriptionAndRepo) {
          ...
          subscriptionAndRepo[1].forEach(function (repo) {
            ...
            if (repo.is_private) {
              store.privateReposReturnedByBitbucket += 1;
              return;
            }
          });
        });
        logger.info('Total number of private repos returned by Bitbucket ',
           store.privateReposReturnedByBitbucket);
        next();
      });
    }
  });
